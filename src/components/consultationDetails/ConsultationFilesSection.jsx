		...
		<>
			...
			>
				...
				<div className="bg-smokeWhite p-4 rounded-lg shadow-md w-full">
					<h3 className="text-2xl text-lightBlue font-semibold mb-2 ">Archivos:</h3>
				  ...
				  <div className=" grid grid-cols-3 auto-rows-auto w-full justify-items-center h-max ">
                    ...
                    <div className="w-full ">
                        {consultationDetails.consultationFiles
                            .filter((file) => !file.filePath.match(/\.(jpeg|jpg|gif|png)$/)).length > 0 ? (
                            consultationDetails.consultationFiles
                                .filter((file) => !file.filePath.match(/\.(jpeg|jpg|gif|png)$/))
                                .map((file, index) => (
                                    <div key={index} className="bg-lightBlue border-[0.1rem] border-solid border-lightBlue w-full p-2 rounded-lg mb-2">
                                        <p className="font-inter font-medium text-md  text-smokeWhite">{file.fileName}</p>
                                    </div>
                                ))
                        ) : (
