...
export const NewConsultationAndHistoryButton = ({
  ...
  return (
    <>
      <div className="flex justify-around mx-4 mb-4 gap-2">
        <Link
          to="/create-consultation"
          className="bg-[#628eff] text-[#f5f5f5] w-max font-bold text-base text-center p-2 rounded-lg active:scale-95 transition-transform transform"
        >
          Añadir nueva consulta
        </Link>
        <button
          className="bg-[#628eff] text-[#f5f5f5] w-max font-bold text-base p-2 rounded-lg active:scale-95 transition-transform transform"
          onClick={() => openModal()}
        >
        ...
        <Modal
          isOpen={isModalOpen}
          onAfterOpen={() =>
            fetchHistoryConsultations(token, setHistoryConsultations)
          }
