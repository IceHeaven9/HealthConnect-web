  ...
  const handleOnSearch = (string, results) => {
    ...
    fetch(`${API_HOST}/my-consultations?title=${string}`, requestOptions)
      ...
      .then((result) => {
        ...
        if (result.length === 0) {
          return fetch(
            `${API_HOST}/my-consultations?description=${string}`,
            requestOptions,
          )
            .then((response) => response.json())
            .then((result) => {
      ...
      <div className="relative min-h-max z-50">
        <div className="flex flex-col p-2 m-1">
          <div className="flex items-center justify-between">
            ...
            <span className="font-ubuntu font-bold text-carbon text-sm pr-6">
              {item.specialityName}
            </span>
          </div>
          <span className="font-ubuntu font-bold text-carbon text-xl">
            {item.name}
          </span>
        </div>
      </div>
